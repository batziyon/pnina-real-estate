...
/**
 * Property Video Upload Component
 *
 * Video management for property media:
 * - Upload videos
 * - View video list
 * - Delete videos
...
export function PropertyVideoUpload({ propertyId, onUploadComplete }: Props) {
  const [uploads, setUploads] = useState<Map<string, VideoUploadState>>(new Map());
  const [videos, setVideos] = useState<PropertyVideoData[]>([]);
  ...
  const loadVideos = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/admin/properties/${propertyId}/videos`);
      if (response.ok) {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = files.filter((file) => {
      if (!file.type.startsWith("video/")) {
        alert(`${file.name}: לא קובץ וידאו`);
        return false;
      }
      
      const allowedTypes = ["video/mp4", "video/webm", "video/quicktime"];
      if (!allowedTypes.includes(file.type)) {
        alert(`${file.name}: סוג קובץ לא נתמך (יש להשתמש ב-MP4, WebM או QuickTime)`);
        return false;
      }
      
      const maxSize = 100 * 1024 * 1024; // 100MB
      if (file.size > maxSize) {
        alert(`${file.name}: הקובץ גדול מדי (מקסימום 100MB)`);
        return false;
      }
    ...
    validFiles.forEach((file) => uploadVideo(file));
  ...
  const uploadVideo = async (file: File) => {
    ...
    try {
      ...
      const intentResponse = await fetch(
        `/api/admin/properties/${propertyId}/videos/generate-upload-url`,
        {
          method: "POST",
      ...
      const { uploadUrl, videoId, storageKey } = intent;
      ...
      const confirmResponse = await fetch(
        `/api/admin/properties/${propertyId}/videos/confirm`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleDelete = async (videoId: string) => {
    if (!confirm("האם אתה בטוח שברצונך למחוק וידאו זה?")) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/properties/${propertyId}/videos/${videoId}`,
        { method: "DELETE" }
      );
  ...
  const handleDragEnd = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/properties/${propertyId}/videos/reorder`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
    ...
    <div className="space-y-4" dir="rtl">
      <div>
        <input
          ...
          className="hidden"
          id={`video-upload-${propertyId}`}
        />
        <label
          htmlFor={`video-upload-${propertyId}`}
        ...
        <p className="mt-2 text-sm text-gray-500">
          MP4, WebM או QuickTime. מקסימום 100MB לקובץ.
        </p>
      </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-medium text-gray-900">
              וידאו בהעלאה ({uploadArray.length})
            </h4>
            {hasCompleted && (
      ...
      {loading ? (
        ...
        <div>
          <h4 className="text-sm font-medium text-gray-900 mb-3">סרטונים ({videos.length})</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {videos.map((video, index) => (
              <div
                ...
                className={`relative group rounded-lg overflow-hidden border-2 border-gray-200 hover:border-gray-300 ${
                  draggedIndex === index ? "opacity-50" : ""
                } cursor-move`}
              >
                <video src={video.url} controls className="w-full h-48 bg-black" />
        ...
        <div className="text-center py-8 text-gray-500">
          <p>אין סרטונים עדיין. העלה סרטון כדי להתחיל.</p>
        </div>
      )}
    </div>
  );
