/**
 * UpdatePropertyInterestStatusUseCase
 *
 * Business rules:
 * - ADMIN: can update any interest
 * - AGENT: can update only if they own the contact OR the property
 * - EDITOR: unauthorized
 * - All status transitions are allowed (no FSM enforcement for MVP)
...
export class UpdatePropertyInterestStatusUseCase {
  constructor(
  ...
  async execute(
    ...
    // 5. Update interest
    return await this.propertyInterestRepository.update(id, input);
  }
}
